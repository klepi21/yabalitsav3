...
export async function POST(request: NextRequest) {
  try {
    ...
    const { parentEmail, parentName, athleteName, month, amount, venueName, venueId } = body;

    if (!parentEmail || !athleteName || !month || !venueId) {
      ...
      <div style="font-family: Inter, system-ui, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; background: #f8fafc;">
        <div style="background: white; border-radius: 12px; padding: 32px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.05);">
          ...
          <div style="margin-bottom: 24px;">
            <p style="color: #1e293b; font-size: 16px; margin: 0;">Αγαπητέ/ή ${parentName || 'Γονέα'},</p>
          ...
          <div style="background: #fef2f2; border: 1px solid #fecaca; border-radius: 12px; padding: 24px; text-align: center; margin-bottom: 24px;">
            ...
            <div style="font-size: 36px; font-weight: 700; color: #dc2626; margin: 0;">&euro;${amount}</div>
            <p style="color: #b91c1c; font-size: 13px; margin: 8px 0 0 0;">${monthName} ${year}</p>
          </div>
          <p style="color: #64748b; font-size: 14px; margin: 0 0 24px 0;">Παρακαλούμε τακτοποιήστε τη συνδρομή το συντομότερο δυνατό. Για οποιαδήποτε απορία, επικοινωνήστε μαζί μας.</p>
