    ...
    <div className="space-y-10 pb-20">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-4 pb-1 border-b border-zinc-50">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold tracking-tight text-zinc-900 mb-0.5">
            {'Στατιστικά Προπονήσεων'}
          </h1>
            <p className="text-2xs font-medium text-zinc-500 tracking-tight">{'Ανάλυση παρουσιών και απόδοσης αθλητών.'}</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col lg:flex-row lg:items-center gap-4">
        <div className="flex flex-wrap items-center gap-3 bg-white p-1.5 rounded-xl border border-zinc-100 shadow-sm">
            ...
            className="h-9 px-4 rounded-lg bg-zinc-50 border-none text-zinc-900 font-medium text-xs focus:outline-none focus:ring-1 focus:ring-zinc-200 min-w-[170px]"
            >
        ...
        {[
            ...
            return (
                <div key={metric.label} className="bg-white p-4 rounded-xl border border-zinc-100 shadow-sm transition-all hover:shadow-md">
                    ...
                    <p className="text-xl font-bold text-zinc-900 mb-0.5 leading-none">{metric.value}</p>
                    <p className="text-2xs font-semibold text-zinc-500">{metric.label}</p>
                </div>
            );
        })}
      </div>
          ...
          <div className="bg-white rounded-xl border border-zinc-100 shadow-sm overflow-hidden">
            <div className="p-4 border-b border-zinc-50 bg-zinc-50/10 flex items-center justify-between">
                <div className="flex items-center gap-3">
                    ...
                    <h3 className="text-base font-semibold text-zinc-900 tracking-tight">{'Κορυφαίες Παρουσίες'}</h3>
                </div>
            ...
            <div className="p-4 space-y-3">
              {athleteStats.filter((a) => a.totalSessions > 0).slice(0, 5).map((athlete, i) => (
                <div key={athlete.id} className="flex items-center gap-3 group">
                  <div className={cn(
                    "h-8 w-8 rounded-lg flex items-center justify-center text-2xs font-semibold transition-all",
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-zinc-900 truncate group-hover:text-emerald-700 transition-colors tracking-tight">{athlete.name}</p>
                    <p className="text-2xs font-medium text-zinc-500">{athlete.totalSessions} {'Προπονήσεις'}</p>
                  </div>
                  <div className={cn("px-3 py-1.5 rounded-lg text-xs font-semibold transition-all group-hover:scale-105 shadow-sm", rateColor(athlete.attendanceRate))}>
          ...
          <div className="bg-white rounded-xl border border-zinc-100 shadow-sm overflow-hidden">
            <div className="p-4 border-b border-zinc-50 bg-zinc-50/10 flex items-center justify-between">
                <div className="flex items-center gap-3">
                    ...
                    <h3 className="text-base font-semibold text-zinc-900 tracking-tight">{'Χαμηλότερες Παρουσίες'}</h3>
                </div>
            ...
            <div className="p-4 space-y-3">
              {[...athleteStats].filter((a) => a.totalSessions > 0).sort((a, b) => a.attendanceRate - b.attendanceRate).slice(0, 5).map((athlete, i) => (
                <div key={athlete.id} className="flex items-center gap-3 group">
                  <div className="h-8 w-8 rounded-lg bg-red-50 flex items-center justify-center text-2xs font-semibold text-red-500 transition-all">
                    {i + 1}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-zinc-900 truncate group-hover:text-red-700 transition-colors tracking-tight">{athlete.name}</p>
                    <p className="text-2xs font-medium text-zinc-500">{athlete.totalSessions} {'Προπονήσεις'}</p>
                  </div>
                  <div className={cn("px-3 py-1.5 rounded-lg text-xs font-semibold transition-all group-hover:scale-105 shadow-sm", rateColor(athlete.attendanceRate))}>
                    {athlete.attendanceRate}%
      ...
      <div className="bg-white rounded-xl border border-zinc-100 shadow-sm overflow-hidden">
        <div className="p-4 border-b border-zinc-50 flex items-center justify-between">
            <div className="flex items-center gap-3">
                ...
                <h3 className="text-base font-semibold text-zinc-900 tracking-tight">{'Αναλυτικά ανά Αθλητή'}</h3>
            </div>
            <div className="px-3 py-1 rounded-lg bg-zinc-50 text-2xs font-semibold text-zinc-500">
        ...
        {athleteStats.length === 0 ? (
          <div className="py-20 text-center">
            ...
            <p className="text-base font-bold text-zinc-500">{'Δεν υπάρχουν δεδομένα για τα επιλεγμένα φίλτρα'}</p>
          </div>
        ) : (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-zinc-50/50">
                  ...
                  <th className="py-3 px-4 text-2xs font-semibold text-zinc-500 text-center">{'Παρών'}</th>
                  <th className="py-3 px-4 text-2xs font-semibold text-zinc-500 text-center">{'Απών'}</th>
                  <th className="py-3 px-4 text-2xs font-semibold text-zinc-500 text-center">{'Καθ.'}</th>
                  <th className="py-3 px-4 text-2xs font-semibold text-zinc-500 text-center">{'Τραυμ.'}</th>
                  <th className="py-3 px-5 text-2xs font-semibold text-zinc-500 lg:min-w-[180px]">{'Παρουσία'}</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-zinc-50">
                {athleteStats.map((athlete, i) => (
                  <tr key={athlete.id} className="group even:bg-zinc-50/70 hover:bg-emerald-50/40 transition-colors">
                    ...
                    <td className="py-3 px-4 text-center">
                      <span className="text-xs font-semibold text-zinc-900">{athlete.totalSessions}</span>
                    </td>
                    <td className="py-3 px-4 text-center">
                      <span className="text-xs font-medium text-emerald-600">{athlete.present}</span>
                    </td>
                    <td className="py-3 px-4 text-center">
                      <span className="text-xs font-medium text-red-500">{athlete.absent}</span>
                    </td>
                    <td className="py-3 px-4 text-center">
                      <span className="text-xs font-medium text-amber-600">{athlete.late}</span>
                    </td>
                    <td className="py-3 px-4 text-center">
                      <span className="text-xs font-medium text-zinc-500">{athlete.injured}</span>
                    </td>
                    <td className="py-3 px-5">
                      <div className="flex items-center gap-3">
                        ...
                        <span className={cn(
                          "text-2xs font-semibold min-w-[35px] text-right rounded-md px-1.5 py-0.5",
                          athlete.totalSessions === 0 ? "text-zinc-400" : rateColor(athlete.attendanceRate)
                        )}>
