        ...
        <div className="flex items-center gap-3.5">
          ...
          <div>
            ...
            <p className="text-2xs font-medium text-zinc-500">
              {'Προγραμματίστε την επόμενη δραστηριότητα'}
            </p>
          </div>
        </div>
      </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-2xl border border-zinc-100 shadow-sm p-6 space-y-6">
              ...
              <div className="space-y-2">
                <Label className="text-2xs font-semibold text-zinc-500">{'Τίτλος Προπόνησης'} *</Label>
                <Input
                  value={form.title}
                  onChange={(e) => setForm((p) => ({ ...p, title: e.target.value }))}
                  placeholder="π.χ. Προπόνηση U12 - Τακτική & Τελειώματα"
                  className="h-11 bg-zinc-50 border-none rounded-xl px-4 font-semibold text-sm focus:bg-white focus:ring-2 focus:ring-emerald-500/20 transition-all"
                />
              </div>

              <div className="space-y-2">
                <Label className="text-2xs font-semibold text-zinc-500">{'Τύπος Δραστηριότητας'} *</Label>
                  ...
                  {TRAINING_TYPES.map((type) => {
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          "h-10 px-3 rounded-xl text-2xs font-semibold transition-all border",
                          isSelected
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Select value={form.squadId} onValueChange={(val: string) => setForm((p) => ({ ...p, squadId: val }))}>
                    <SelectTrigger className="h-11 px-4 bg-zinc-50 border-none rounded-xl font-semibold text-sm focus:ring-2 focus:ring-emerald-500/20">
              ...
              <div className="space-y-3 pt-4 border-t border-zinc-100">
                ...
                {isRecurring && (
                  <div className="bg-emerald-50/50 border border-emerald-100 rounded-xl p-4 space-y-4 animate-in fade-in slide-in-from-top-2">
                    <div className="space-y-2">
                      <Label className="text-2xs font-semibold text-emerald-600">
                        {'Ημέρες'}
                      </Label>
                      <div className="grid grid-cols-7 gap-1.5">
                        {DAYS.map((day, i) => (
                          <button
                            ...
                            className={cn(
                              "h-9 rounded-lg text-2xs font-semibold transition-all",
                    ...
                    {recurringDays.length > 0 && (
                      <div className="flex items-center gap-2 p-2.5 bg-white rounded-lg border border-emerald-100">
                        ...
                        <p className="text-2xs font-medium text-emerald-700">
                          <span className="font-semibold">{getRecurringDates().length}</span> προπονήσεις
              ...
              <div className="flex items-center justify-between">
                ...
                {drills.length > 0 && (
                  <div className="flex items-center gap-1.5 px-3 py-1.5 bg-violet-50 rounded-lg">
                    ...
                    <span className="text-xs font-semibold text-violet-700">{totalDrillMinutes} λ.</span>
                  </div>
                )}
              </div>
                ...
                <div className="space-y-2">
                  {drills.map((drill, i) => (
                    <div key={i} className="flex items-center gap-3 p-3 rounded-xl border border-zinc-100 bg-zinc-50/50 hover:bg-white hover:shadow-sm transition-all">
                      <div className="h-8 w-8 rounded-lg bg-white border border-zinc-100 flex items-center justify-center text-sm font-semibold text-zinc-500">
                        {i + 1}
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <p className="text-sm font-semibold text-zinc-900 truncate">{drill.name}</p>
                          <span className="px-2 py-0.5 bg-violet-100 text-violet-700 rounded-md text-2xs font-semibold shrink-0">{drill.duration} λ.</span>
                        </div>
                        {drill.description && <p className="text-2xs text-zinc-500 truncate mt-0.5">{drill.description}</p>}
                      </div>
                      <button type="button" onClick={() => removeDrill(i)} className="h-7 w-7 rounded-lg flex items-center justify-center text-zinc-400 hover:text-red-500 hover:bg-red-50 transition-all">
              ...
              <div className="bg-zinc-50/50 border border-dashed border-zinc-200 rounded-xl p-4 space-y-3">
                <div className="grid grid-cols-1 sm:grid-cols-[1fr_100px] gap-3">
                  <div className="space-y-1.5">
                    <Label className="text-2xs font-semibold text-zinc-500">{'Άσκηση'}</Label>
                    <Input
                      placeholder="π.χ. Προθέρμανση με μπάλα..."
                      value={newDrill.name}
                      onChange={(e) => setNewDrill((p) => ({ ...p, name: e.target.value }))}
                      className="h-10 bg-white border-none rounded-lg px-3 font-semibold text-sm focus:ring-2 focus:ring-violet-500/20"
                    />
                  </div>
                  <div className="space-y-1.5">
                    <Label className="text-2xs font-semibold text-zinc-500">{'Λεπτά'}</Label>
                    <Input
                      type="number"
                      value={newDrill.duration}
                      onChange={(e) => setNewDrill((p) => ({ ...p, duration: Number(e.target.value) || 0 }))}
                      className="h-10 bg-white border-none rounded-lg px-3 text-center font-semibold text-sm focus:ring-2 focus:ring-violet-500/20"
                      min={1}
                    />
                  </div>
                </div>
                ...
                <Button
                  ...
                  className="h-10 w-full rounded-lg border-zinc-200 font-medium text-xs hover:bg-white"
                >
          ...
          <div className="space-y-6">
                  ...
                  {coaches.filter((c) => c.id !== form.coachId).map((coach) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className={cn("text-xs font-medium", isSelected ? "text-blue-900" : "text-zinc-500")}>
                          {coach.displayName}
                        </span>
                      </label>
                    );
            ...
            <div className="bg-white rounded-2xl border border-zinc-100 shadow-sm p-6 space-y-4">
              ...
              <textarea
                ...
                rows={5}
                className="w-full rounded-xl bg-zinc-50 border-none p-4 font-semibold text-sm focus:outline-none focus:ring-2 focus:ring-amber-500/20 placeholder:text-zinc-500 resize-none transition-all"
            ...
            <div className="flex flex-col gap-3">
              ...
              >
                ...
                {isSaving
                  ...
                  : isRecurring && recurringDays.length > 0
                    ? `Δημιουργία ${getRecurringDates().length} Προπονήσεων`
              ...
              <Button
                type="button"
                variant="ghost"
                className="h-10 w-full rounded-xl font-semibold text-zinc-500 hover:text-zinc-600 hover:bg-zinc-100 text-sm"
                onClick={() => router.back()}
              >
                {'Ακύρωση'}
              </Button>
